...
for i in range(data_false.shape[0]):
    ...
    x_false[i, :] = data_false[i, 1]  # (16838, 50)
...
x_train = np.concatenate([x_false_train, x_true_train], axis=0)  #.astype(dtype="int64")  # (7265, 50, 20)
x_test = np.concatenate([x_false_test, x_true_test], axis=0)  #.astype(dtype="int64")
y_train = np.concatenate([y_false_train, y_true_train], axis=0).astype(dtype="int64")#.reshape([-1, 1])
y_test = np.concatenate([y_false_test, y_true_test], axis=0).astype(dtype="int64")#.reshape([-1, 1])
...
# df.to_excel(r'.\data\traindata.xlsx')
# # 测试集
# test = np.concatenate([y_test, x_test], axis=1)
# df = pd.DataFrame(test)
# df.to_excel(r'.\data\testdata.xlsx')
...
class NN(nn.Module):
    ...
    def forward(self, x, batchsize=batch_size):
        ...
        # x_cnn = x_cat
        # print(x_cnn.shape, "cnn")
        # x = torch.sum(x_cnn, dim=2)  # 在CNN使用Vasvani注意力
