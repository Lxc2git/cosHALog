# python3.9
# time:2024/11/11
# CNN:scaled dot-product attention mechanism  GRU:Swiss attention mechanism
...
length = 50
x_dimens = 30
# data_false = pd.read_excel(r'BGL\BGL_false.xlsx')
# data_true = pd.read_excel(r'BGL\BGL_true.xlsx')
data_false = pd.read_csv(r'BGL\random_train.csv', names=["序列", "标签"])
data_true = pd.read_csv(r'BGL\random_test.csv', names=["序列", "标签"])
...
for i in range(data_false.shape[0]):
    ...
    if len(data_false[i, 0]) > length:
        data_false[i, 0] = data_false[i, 0][:length]
    else:
        data_false[i, 0] = data_false[i, 0] + [0 for x in range(length - len(data_false[i, 0]))]
    x_false[i, :] = data_false[i, 0]
for i in range(data_true.shape[0]):
    data_true[i, 0] = data_true[i, 0].split(" ")
    data_true[i, 0].pop()
    data_true[i, 0] = [int(x) for x in data_true[i, 0]]
    if len(data_true[i, 0]) > length:
        data_true[i, 0] = data_true[i, 0][:length]
    else:
...
x_train = sequence_false
x_test = sequence_true
y_train = y_false.astype(np.int64)
y_test = y_true.astype(np.int64)
...
class NN(nn.Module):
    def __init__(self):
        ...
        self.attention1 = nn.Linear(channel_size, h_gru)
        ...
        self.dotline = nn.Linear(channel_size, channel_size)
    ...
    def forward(self, x, batchsize=batch_size):
        ...
        # x1 = torch.matmul(F.softmax(torch.matmul(x1.reshape([batchsize, length, channel_size]), x1)/math.sqrt(channel_size), dim=1),x1.reshape([batchsize, length, channel_size]))
        # x1 = x1.reshape([batchsize, channel_size, length])
        x1 = self.dotproduct(x1,channel_size,batchsize)
        x2 = self.conv2(x)
        x2 = F.leaky_relu(x2, negative_slope=0.1)
        # x2 = torch.matmul(
        #     F.softmax(torch.matmul(x2.reshape([batchsize, length, channel_size]), x2) / math.sqrt(channel_size), dim=1),
        #     x2.reshape([batchsize, length, channel_size]))
        # x2 = x2.reshape([batchsize, channel_size, length])
        x2 = self.dotproduct(x2, channel_size, batchsize)
        x3 = self.conv3(x)
        x3 = F.leaky_relu(x3, negative_slope=0.1)
        # x3 = torch.matmul(
        #     F.softmax(torch.matmul(x3.reshape([batchsize, length, channel_size]), x3) / math.sqrt(channel_size), dim=1),
        #     x3.reshape([batchsize, length, channel_size]))
        # x3 = x3.reshape([batchsize, channel_size, length])
        x3 = self.dotproduct(x3, channel_size, batchsize)
        ...
        for i in range(x.shape[2]):
            if i == 0:
                attn = F.tanh(self.attention1(x[:, :, i].reshape([x.shape[0], 1, channel_size])))
            else:
                attn = torch.cat([attn, F.tanh(self.attention1(x[:, :, i].reshape([x.shape[0], 1, channel_size])))], dim=1)  # (batch_size, 50, 128)
        x_gru = torch.sum(x4 * attn, dim=1)  # (batch_size, 3*channel_size)
    ...
    def attention2(self, gru_output, seq_len):
        ...
        attn_tanh = torch.tanh(torch.mm(output_reshape, self.w_omega.to(device))/math.sqrt(1))
    ...
    def dotproduct(self, x, h, batch):
        y = torch.matmul(
            F.softmax(torch.matmul(x.reshape([batch, length, h]), x) / h, dim=1),
            x.reshape([batch, length, h]))
        # x = x.reshape([batch, length, h])
        # y = torch.matmul(
        #     F.softmax(torch.matmul(self.dotline(x), self.dotline(x).reshape([batch, h, length])) / math.sqrt(h), dim=1),
        #     self.dotline(x))
