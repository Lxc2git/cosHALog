...
class NN(nn.Module):
    def __init__(self):
        ...
        self.maxpool1 = nn.MaxPool1d(length, stride=1)
        self.maxpool2 = nn.MaxPool1d(length, stride=1)
    ...
    def forward(self, x, batchsize=batch_size):
        ...
        # x_gru = self.maxpool1(x4.reshape([batchsize, h_gru, length]))
        x4 = self.dotproduct1(x4, h_gru, batchsize)  # 注意力
        ...
        # print(x_cat.shape)
        x_cnn = self.maxpool2(x_cat)
        # x_cnn = self.attention2(x_cat.reshape([batchsize, length, 3*channel_size]), length)  # (batch_size, h_gru) 注意力

        # x_gru = self.attention1(x4, length)  # (batch_size, h_gru)
        # print(x_cnn.shape, "111", x_gru.shape)
        x = torch.cat([x_cnn.reshape([batchsize, 3*channel_size]), x_gru], dim=1)
